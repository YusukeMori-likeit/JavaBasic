...
public class PTra10_04 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0;i < users.length;i++) {
			...
			users[i] = new User();
			users[i].userId = Integer.parseInt(word[0]);
			users[i].userNm = word[1];
			users[i].mail = word[2];
			users[i].password = word[3];
		}
		//----------- ループ end

		// ★ usersに格納されている全てのインスタンスの、フィールドuserId, userNm, mail, passwordを出力してください
		for(int j = 0;j < users.length;j++) {
			System.out.println(users[j].userId + "," + users[j].userNm + "," + users[j].mail + "," + users[j].password);
		}
